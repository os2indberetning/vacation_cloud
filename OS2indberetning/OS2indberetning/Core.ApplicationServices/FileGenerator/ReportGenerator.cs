...
namespace Core.ApplicationServices.FileGenerator
{
    public class ReportGenerator
    {
        ...
        public ReportGenerator(IGenericRepository<DriveReport> reportRepo, IReportFileWriter fileWriter, IConfiguration config)
        {
            ...
            foreach (var cpr in usersToReimburse.Keys)
            {
                ...
                foreach (var driveReport in driveReports)
                {
                            ...
                            || !(driveReport.AccountNumber == currentAccountNumber))
                    {
                        ...
                        {
                            fileRecords.Add(new FileRecord(currentDriveReport, cpr,_config));
                        }
